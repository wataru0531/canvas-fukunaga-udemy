...
function draw(){
  ...
  // ctx.beginPath
  // → 新しいパス(path)の開始を宣言
  //   これにより、現在のパスがリセットされ、新しい描画操作を独立して行うことができるようになる
  //   パスをリセットするのみげ限定されて、描画スタイル(strokeStyle, fillStyle)はリセットしない。
  ctx.beginPath(); // ここから新しい描画処理を始める(1つの図形に1つつけたほうがいい)
  ctx.rect(100, 100, 100, 100); // rect(x座表, y座表, 幅, 高さ)
  ...
  // save ... ctxの内部ではこれまでの描画コンテキストの状態がスタックに、スナップショットのように保存される。
  ...
  ctx.ellipse(500, 350, 100, 50, Math.PI/3, 0, Math.PI * 2);
  ...
  ctx.lineWidth = 3;
  ctx.beginPath();
  ctx.moveTo(100, 550); // moveTo 描画の開始位置
  ...
  ctx.lineTo(550, 720); // 開始位置から、xが550, yが720の地点にまで動く
  ctx.stroke();
  // ②
  ctx.beginPath();
  ctx.moveTo(275, 750);
